...
function useLearningSpaces(role:Role,user:User){
 const [classes,setClasses]=useState<DetailClassroom[]>([]),[memberships,setMemberships]=useState<Membership[]>([]),[pending,setPending]=useState<JoinRequest[]>([]);
...
export function ProgressDetail({role,user,onOpen,onCountChange}:{role:Role;user:User;onOpen:(c:DetailClassroom)=>void;onCountChange:(count:number)=>void}){
 const {classes,memberships}=useLearningSpaces(role,user);const items=role==='teacher'?classes:memberships;const overall=items.length?Math.round(items.reduce((n,x)=>n+(x.progress||0),0)/items.length):0;const learners=classes.reduce((n,c)=>n+(c.students||0),0);
 useEffect(()=>onCountChange(items.length),[items.length,onCountChange]);
 const open=(item:DetailClassroom|Membership)=>onOpen(role==='teacher'?item as DetailClassroom:{id:(item as Membership).classId,name:(item as Membership).className,subject:(item as Membership).className,code:(item as Membership).code,teacherId:(item as Membership).teacherId,teacherName:(item as Membership).teacherName,students:0,progress:item.progress||0});
 return <><header className="detail-page-head"><div><span className="kicker">Performance</span><h1>{role==='teacher'?'Class progress':'My progress'}</h1><p>{role==='teacher'?'See how learners are progressing across every classroom.':'Track your learning progress across all active classes.'}</p></div></header><section className="progress-summary"><article className="progress-primary"><small>OVERALL PROGRESS</small><strong>{overall}<span>%</span></strong><Progress value={overall}/><p>{items.length?`Across ${items.length} active classroom${items.length>1?'s':''}`:'Progress will appear after you join a classroom.'}</p></article><article><span><BookOpen/></span><small>CLASSROOMS</small><strong>{items.length}</strong></article><article><span><Users/></span><small>{role==='teacher'?'LEARNERS':'TASKS DUE'}</small><strong>{role==='teacher'?learners:memberships.reduce((n,m)=>n+(m.tasks||0),0)}</strong></article></section><section className="detail-panel progress-breakdown"><div className="panel-head"><div><span className="kicker">Breakdown</span><h2>Progress by classroom</h2></div></div>{items.length===0?<div className="detail-empty"><BarChart3/><h3>No progress data yet</h3><p>{role==='teacher'?'Create a classroom to start tracking learners.':'Join a classroom and complete activities to build progress.'}</p></div>:items.map((item,i)=>{const name=role==='teacher'?(item as DetailClassroom).name:(item as Membership).className;const subtitle=role==='teacher'?`${(item as DetailClassroom).students||0} learners`:(item as Membership).teacherName;return <button className="progress-row" key={item.id} onClick={()=>open(item)}><span className={`progress-number ${colours[i%3]}`}>0{i+1}</span><div><div><b>{name}</b><small>{subtitle}</small></div><Progress value={item.progress||0}/></div><strong>{item.progress||0}%</strong><ChevronRight/></button>})}</section></>;
}
