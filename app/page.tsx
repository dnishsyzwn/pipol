...
function TeacherDashboard({ onSwitch, onView, onExit }: { onSwitch: (r: Role)=>void; onView: (v: View)=>void; onExit:()=>void }) {
  ...
  const [createOpen, setCreateOpen] = useState(false); const approve=(n:string)=>setRequests(requests.filter(r=>r.name!==n));
  return <AppShell role="teacher" onExit={onExit}><Topbar role="teacher" onSwitch={onSwitch}/>
    ...
    <Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogContent className="modal-card"><DialogHeader><DialogTitle>Create a classroom</DialogTitle><DialogDescription>Give your new learning space a clear name.</DialogDescription></DialogHeader><label className="form-label">Classroom name<Input defaultValue="Bahasa Melayu · Form 4"/></label><label className="form-label">Subject<Input defaultValue="Bahasa Melayu"/></label><DialogFooter><Button variant="outline" onClick={()=>setCreateOpen(false)}>Cancel</Button><Button onClick={()=>setCreateOpen(false)}><Plus/> Create classroom</Button></DialogFooter></DialogContent></Dialog>
  </AppShell>;
}

function StudentDashboard({ onSwitch, onView, onExit }: { onSwitch: (r: Role)=>void; onView: (v: View)=>void; onExit:()=>void }) {
  const [joinOpen,setJoinOpen]=useState(false); const [pending,setPending]=useState(true);
  ...
  return <AppShell role="student" onExit={onExit}><Topbar role="student" onSwitch={onSwitch}/>
    ...
    {pending && <div className="pending-banner"><span><Clock3/></span><div><b>Join request pending</b><p>Cikgu Aina will review your request for <strong>Science · Form 3</strong>.</p></div><button onClick={()=>setPending(false)}><X/></button></div>}
