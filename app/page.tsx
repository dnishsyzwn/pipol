...
import { AlertCircle, ArrowLeft, ArrowRight, BarChart3, BookOpen, Bot, Calendar, Camera, Check, CheckCircle2, ChevronRight, Clock3, Copy, FileQuestion, GraduationCap, LayoutDashboard, LoaderCircle, Lock, LogIn, LogOut, MoreHorizontal, Pencil, Plus, Search, Send, Sparkles, Trash2, Unlock, Users, WandSparkles, X } from 'lucide-react';
...
function computeSubmissionStats(sub: Partial<SubmissionData> | any, ex: any): { correct: number; wrong: number; questionResults: QuestionResult[] } {
 const qList: any[] = ex?.questions?.length ? ex.questions : (ex?.question ? [{ question: ex.question, answer: ex.answer || '', points: 2 }] : []);

 // 1. If submission has explicit questionResults
 if (sub?.questionResults && Array.isArray(sub.questionResults) && sub.questionResults.length > 0) {
  const c = sub.questionResults.filter((r: QuestionResult) => r.isCorrect).length;
  const w = sub.questionResults.filter((r: QuestionResult) => !r.isCorrect).length;
 ...
 if (qList.length > 0) {
  ...
  const results: QuestionResult[] = qList.map((q, idx) => {
   const userA = (sub?.answers && sub.answers[idx] !== undefined ? String(sub.answers[idx]) : '').trim();
   ...
   let isCorrect = false;
   if (userALow && expALow && (userALow === expALow || expALow.includes(userALow) || userALow.includes(expALow))) {
    isCorrect = true;
    c++;
   } else {
    w++;
   ...
   return {
    ...
    studentAnswer: userA,
    isCorrect,
    pointsEarned: isCorrect ? pts : (userA ? Math.max(1, Math.round(pts * 0.5)) : 0),
    pointsPossible: pts
   };
  });
  ...
  if (c === 0 && sub?.score > 0) {
   const totalPossible = sub.totalPoints || qList.reduce((sum: number, q: any) => sum + (Number(q.points) || 1), 0);
   if (sub.score >= totalPossible) {
    c = qList.length;
    w = 0;
   } else {
    c = Math.max(1, Math.min(qList.length, Math.round((sub.score / totalPossible) * qList.length)));
 ...
 if (typeof sub?.totalCorrect === 'number' && typeof sub?.totalWrong === 'number' && (sub.totalCorrect + sub.totalWrong > 0)) {
  return { correct: sub.totalCorrect, wrong: sub.totalWrong, questionResults: [] };
 }
...
function AppShell({role,user,onExit,onNavigate,children,active='dashboard',classCount=0}:{role:Role;user:User;onExit:()=>void;onNavigate?:(target:NavTarget)=>void;children:React.ReactNode;active?:View;classCount?:number}){
 const [profileOpen,setProfileOpen]=useState(false),[profileName,setProfileName]=useState(user.displayName||''),[profilePhoto,setProfilePhoto]=useState(user.photoURL||''),[profileFile,setProfileFile]=useState<File|null>(null),[profilePreview,setProfilePreview]=useState(user.photoURL||''),[profileSaving,setProfileSaving]=useState(false),[profileError,setProfileError]=useState('');
 ...
 const go=(target:NavTarget)=>{if(onNavigate){onNavigate(target);return}window.dispatchEvent(new CustomEvent<NavTarget>('slearn:navigate',{detail:target}))};
 ...
 const saveProfile=async()=>{const cleanName=profileName.trim();if(!cleanName){setProfileError('Please enter your name.');return}setProfileSaving(true);setProfileError('');try{let photoURL=profilePhoto;if(profileFile){const extension=profileFile.type==='image/png'?'png':profileFile.type==='image/webp'?'webp':'jpg';const photoRef=ref(storage,`users/${user.uid}/profile/avatar-${Date.now()}.${extension}`);await uploadBytes(photoRef,profileFile,{contentType:profileFile.type});photoURL=await getDownloadURL(photoRef)}await updateProfile(user,{displayName:cleanName,photoURL:photoURL||null});await setDoc(doc(db,'users',user.uid),{displayName:cleanName,photoURL:photoURL||'',updatedAt:serverTimestamp()},{merge:true});setProfileName(cleanName);setProfilePhoto(photoURL||'');setProfileFile(null);window.dispatchEvent(new Event('slearn:profile-updated'));setProfileOpen(false)}catch(e){setProfileError(friendlyError(e))}finally{setProfileSaving(false)}};
 const avatar=(className?:string)=><span className={className}>{profilePhoto?<img src={profilePhoto} alt=""/>:initials(profileName||user.displayName)}</span>;
 return <div className="app-shell"><aside className="sidebar"><Brand/><nav aria-label="Main navigation"><button className={active==='dashboard'?'active':''} onClick={()=>go('overview')} title="Overview" aria-label="Overview"><LayoutDashboard/><span>Overview</span></button><button className={active==='classroom'||active==='quiz'||active==='exercise'?'active':''} onClick={()=>go('classes')} title="Classrooms" aria-label="Classrooms"><BookOpen/><span>Classrooms</span><b>{classCount}</b></button><button onClick={()=>go('progress')} title="Progress" aria-label="Progress"><BarChart3/><span>Progress</span></button></nav><div className="sidebar-foot"><button type="button" className="mini-profile profile-trigger" onClick={openProfile} aria-label="Edit profile" title="Edit profile">{avatar()}<div><b>{profileName||'SLearn user'}</b><small>{role}</small></div></button><button onClick={onExit} aria-label="Sign out" title="Sign out"><LogOut/></button></div></aside><div className="mobile-bar"><Brand/><button type="button" className="mobile-profile" onClick={openProfile} aria-label="Edit profile">{avatar()}</button></div><section className="main-stage">{children}</section><Dialog open={profileOpen} onOpenChange={setProfileOpen}><DialogContent className="modal-card profile-modal"><DialogHeader><DialogTitle>Edit your profile</DialogTitle><DialogDescription>Update how your name and photo appear in SLearn.</DialogDescription></DialogHeader><div className="profile-editor"><div className="profile-photo-preview">{profilePreview?<img src={profilePreview} alt="Profile preview"/>:<span>{initials(profileName)}</span>}<label title="Choose profile picture"><Camera/><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>choosePhoto(e.target.files?.[0])}/></label></div><label className="form-label">Display name<Input value={profileName} maxLength={60} onChange={e=>setProfileName(e.target.value)} placeholder="Your name"/></label><p className="profile-help">JPG, PNG or WebP · maximum 5 MB</p>{profileError&&<p className="auth-error">{profileError}</p>}</div><DialogFooter><Button variant="outline" onClick={()=>setProfileOpen(false)} disabled={profileSaving}>Cancel</Button><Button className="primary-action" onClick={saveProfile} disabled={profileSaving}>{profileSaving?<><LoaderCircle/> Saving…</>:<><Check/> Save profile</>}</Button></DialogFooter></DialogContent></Dialog></div>
}
...
function TeacherDashboard({user,onView,onExit,onSelectClass}:{user:User;onView:(v:View)=>void;onExit:()=>void;onSelectClass:(c:ClassroomData)=>void}){
 ...
 const confirmDelete=async()=>{if(!deleteTarget)return;setDeleting(true);setDeleteError('');try{const classId=deleteTarget.id;const [membersSnap,requestsSnap,exercisesSnap]=await Promise.all([getDocs(collection(db,'classrooms',classId,'members')),getDocs(collection(db,'classrooms',classId,'requests')),getDocs(collection(db,'classrooms',classId,'exercises'))]);const batch=writeBatch(db);membersSnap.docs.forEach(d=>{batch.delete(doc(db,'users',d.id,'memberships',classId));batch.delete(doc(db,'classrooms',classId,'members',d.id))});requestsSnap.docs.forEach(d=>{batch.delete(doc(db,'users',d.id,'joinRequests',classId));batch.delete(doc(db,'classrooms',classId,'requests',d.id))});for(const exDoc of exercisesSnap.docs){const subsSnap=await getDocs(collection(db,'classrooms',classId,'exercises',exDoc.id,'submissions'));subsSnap.docs.forEach(subDoc=>batch.delete(doc(db,'classrooms',classId,'exercises',exDoc.id,'submissions',subDoc.id)));batch.delete(doc(db,'classrooms',classId,'exercises',exDoc.id));}batch.delete(doc(db,'classrooms',classId));await batch.commit();setDeleteTarget(null)}catch(e){setDeleteError(friendlyError(e))}finally{setDeleting(false)}};
...
function Classroom({role,user,classroom,onBack,onQuiz,onStartExercise,onExit,onClassUpdated,onClassDeleted}:{role:Role;user:User;classroom:ClassroomData;onBack:()=>void;onQuiz:()=>void;onStartExercise:(ex:any)=>void;onExit:()=>void;onClassUpdated?:(c:ClassroomData)=>void;onClassDeleted?:()=>void}){
 ...
 const [exercises,setExercises]=useState<{id:string;title:string;deadline?:string|null;allowLateSubmissions?:boolean;question?:string;questions?:any[];questionCount?:number;enhanced?:boolean}[]>([]);
 ...
 const confirmDelete=async()=>{setDeleting(true);setDeleteError('');try{const classId=classroom.id;const [membersSnap,requestsSnap,exercisesSnap]=await Promise.all([getDocs(collection(db,'classrooms',classId,'members')),getDocs(collection(db,'classrooms',classId,'requests')),getDocs(collection(db,'classrooms',classId,'exercises'))]);const batch=writeBatch(db);membersSnap.docs.forEach(d=>{batch.delete(doc(db,'users',d.id,'memberships',classId));batch.delete(doc(db,'classrooms',classId,'members',d.id))});requestsSnap.docs.forEach(d=>{batch.delete(doc(db,'users',d.id,'joinRequests',classId));batch.delete(doc(db,'classrooms',classId,'requests',d.id))});for(const exDoc of exercisesSnap.docs){const subsSnap=await getDocs(collection(db,'classrooms',classId,'exercises',exDoc.id,'submissions'));subsSnap.docs.forEach(subDoc=>batch.delete(doc(db,'classrooms',classId,'exercises',exDoc.id,'submissions',subDoc.id)));batch.delete(doc(db,'classrooms',classId,'exercises',exDoc.id));}batch.delete(doc(db,'classrooms',classId));await batch.commit();setDeleteOpen(false);if(onClassDeleted)onClassDeleted();else onBack()}catch(e){setDeleteError(friendlyError(e))}finally{setDeleting(false)}};

 const [analyticsMap, setAnalyticsMap] = useState<Record<string, ExerciseAnalytics>>({});
 const [activeAnalyticsEx, setActiveAnalyticsEx] = useState<{ id: string; title: string; deadline?: string | null; allowLateSubmissions?: boolean; questions?: any[]; question?: string; questionCount?: number } | null>(null);
 ...
 useEffect(() => {
  ...
  const unsubs = exercises.map(ex => {
   const qList = ex.questions?.length ? ex.questions : (ex.question ? [{ question: ex.question, answer: (ex as any).answer || '', points: 2 }] : []);
   const subCol = collection(db, 'classrooms', classroom.id, 'exercises', ex.id, 'submissions');
   const subQuery = role === 'student' ? query(subCol, where('studentId', '==', user.uid)) : subCol;

   return onSnapshot(subQuery, snap => {
    const rawSubs: SubmissionData[] = snap.docs.map(d => ({ id: d.id, ...d.data() } as SubmissionData));
    ...
    const qStats = qList.map((q: any, qIdx: number) => {
     ...
     subs.forEach(s => {
      ...
      const qRes = stats.questionResults.find(r => r.questionIdx === qIdx);
      if (qRes) {
     ...
     return {
      ...
      accuracyRate: (cCount + wCount) > 0 ? Math.round((cCount / (cCount + wCount)) * 100) : 0
     };
    });
    ...
    const accuracyRate = grandTotal > 0 ? Math.round((totalCorrect / grandTotal) * 100) : 0;
 ...
 const totalClassSubs = Object.values(analyticsMap).reduce((n, a) => n + a.totalAnswered, 0);
 const totalClassCorrect = Object.values(analyticsMap).reduce((n, a) => n + a.totalCorrect, 0);
 const totalClassWrong = Object.values(analyticsMap).reduce((n, a) => n + a.totalWrong, 0);
 const overallTotalAnswers = totalClassCorrect + totalClassWrong;
 const classAccuracy = overallTotalAnswers > 0 ? Math.round((totalClassCorrect / overallTotalAnswers) * 100) : 0;

 const completedExercisesCount = exercises.filter(ex => analyticsMap[ex.id]?.submissions.some(s => s.studentId === user.uid || s.id === user.uid)).length;
 const studentMasteryPct = exercises.length > 0 ? Math.round((completedExercisesCount / exercises.length) * 100) : 0;
 ...
 const saveEditExercise = async () => {
  ...
  setEditExError('');
  try {
   await updateDoc(doc(db, 'classrooms', classroom.id, 'exercises', editExTarget.id), {
 ...
 const confirmDeleteExercise = async () => {
  ...
  try {
   ...
   const subsSnap = await getDocs(collection(db, 'classrooms', classroom.id, 'exercises', exId, 'submissions'));
   const batch = writeBatch(db);
   subsSnap.docs.forEach(subDoc => batch.delete(doc(db, 'classrooms', classroom.id, 'exercises', exId, 'submissions', subDoc.id)));
 ...
 return <AppShell role={role} user={user} onExit={onExit} active="classroom" classCount={1}><div className="detail-top"><button onClick={onBack}><ArrowLeft/> Back to overview</button><span className="sdg-pill">{currentClass.code}</span></div><div className="classroom-title"><div><span className="kicker">Classroom</span><h1>{currentClass.name}</h1><p>{role==='teacher'?`${currentClass.students||0} / ${currentClass.maxStudents||30} learners · ${currentClass.subject}`:`${currentClass.teacherName} · You’re approved`}</p></div>{role==='teacher'&&<div className="classroom-actions"><Button variant="outline" size="sm" onClick={()=>{setEditName(currentClass.name);setEditSubject(currentClass.subject||'');setEditMaxStudents(String(currentClass.maxStudents||30));setEditError('');setEditOpen(true)}}><Pencil style={{width:14,height:14}}/> Edit class</Button><Button variant="outline" size="sm" onClick={()=>setDeleteOpen(true)} style={{color:'#dc2626',borderColor:'#fca5a5'}}><Trash2 style={{width:14,height:14}}/> Delete</Button><Button onClick={onQuiz} className="primary-action"><Plus/> Create exercise</Button></div>}</div><div className="classroom-layout"><section className="panel activity-panel"><div className="panel-head"><div><span className="kicker">Learning activities</span><h2>{exercises.length?`${exercises.length} Exercise${exercises.length>1?'s':''}`:'No exercises yet'}</h2></div></div>{exercises.length?<div style={{display:'grid',gap:'0.85rem',marginTop:'1rem'}}>{exercises.map((ex,i)=>{const count=ex.questionCount||ex.questions?.length||1;const firstQ=ex.question||(ex.questions&&ex.questions[0]?.question)||'Exercise checkpoint';const stats=analyticsMap[ex.id];const mySub=role==='student'?stats?.submissions.find(s=>s.studentId===user.uid||s.id===user.uid):null;const isDone=Boolean(mySub);const mySubStats=mySub?computeSubmissionStats(mySub,ex):null;const hasAnswered=stats&&stats.totalAnswered>0;const due=formatDeadline(ex.deadline);const allowsLate=ex.allowLateSubmissions!==false;const isClosedForStudent=role==='student'&&!isDone&&due.isPast&&!allowsLate;return <div key={ex.id||i} className="ex-card" style={isClosedForStudent?{opacity:0.85,background:'#fbfaf9'}:undefined}><div><div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginBottom:'0.4rem',flexWrap:'wrap'}}><FileQuestion style={{width:'18px',height:'18px',color:'#111'}}/><b style={{fontSize:'1.05rem'}}>{ex.title}</b>{role==='teacher'&&<div className="card-actions" style={{display:'inline-flex',alignItems:'center',gap:'4px'}}><button type="button" className="action-icon-btn" title="Edit exercise" onClick={e=>{e.stopPropagation();openEditExercise(ex)}} aria-label={`Edit ${ex.title}`} style={{width:26,height:26}}><Pencil style={{width:12,height:12}}/></button><button type="button" className="action-icon-btn delete-btn" title="Delete exercise" onClick={e=>{e.stopPropagation();openDeleteExercise(ex)}} aria-label={`Delete ${ex.title}`} style={{width:26,height:26}}><Trash2 style={{width:12,height:12}}/></button></div>}<span style={{background:'#eef2ee',color:'#44554b',fontSize:'0.68rem',padding:'0.2rem 0.55rem',borderRadius:'99px',fontWeight:600}}>{count} Question{count>1?'s':''}</span>{ex.enhanced&&<span style={{background:'#d9f1e5',color:'#111',fontSize:'0.65rem',padding:'0.2rem 0.5rem',borderRadius:'99px',display:'inline-flex',alignItems:'center',gap:'0.2rem'}}><Sparkles style={{width:'10px',height:'10px'}}/> AI Enhanced</span>}{due.formatted&&(!isDone?(due.isPast?<span style={{background:allowsLate?'#fef2f2':'#fee2e2',color:allowsLate?'#991b1b':'#7f1d1d',border:`1px solid ${allowsLate?'#fecaca':'#fca5a5'}`,fontSize:'0.68rem',padding:'0.2rem 0.55rem',borderRadius:'99px',fontWeight:600,display:'inline-flex',alignItems:'center',gap:'0.25rem'}}>{allowsLate?<AlertCircle style={{width:'11px',height:'11px'}}/>:<Lock style={{width:'11px',height:'11px'}}/>} {allowsLate?`Past Due · ${due.formatted}`:`Closed · ${due.formatted}`}</span>:due.isUrgent?<span style={{background:'#fef3c7',color:'#92400e',border:'1px solid #fde68a',fontSize:'0.68rem',padding:'0.2rem 0.55rem',borderRadius:'99px',fontWeight:600,display:'inline-flex',alignItems:'center',gap:'0.25rem'}}><Clock3 style={{width:'11px',height:'11px'}}/> Due Soon · {due.formatted}</span>:<span style={{background:'#f3f4f6',color:'#374151',border:'1px solid #e5e7eb',fontSize:'0.68rem',padding:'0.2rem 0.55rem',borderRadius:'99px',fontWeight:600,display:'inline-flex',alignItems:'center',gap:'0.25rem'}}><Calendar style={{width:'11px',height:'11px'}}/> Due {due.formatted}</span>):<span style={{background:'#f8faf7',color:'#55655a',border:'1px solid #e5eae6',fontSize:'0.68rem',padding:'0.2rem 0.55rem',borderRadius:'99px',fontWeight:500,display:'inline-flex',alignItems:'center',gap:'0.25rem'}}><Calendar style={{width:'11px',height:'11px'}}/> Due {due.formatted}</span>)}{ex.deadline&&<span style={{fontSize:'0.66rem',padding:'0.18rem 0.5rem',borderRadius:'99px',background:allowsLate?'#f0fdf4':'#fef2f2',color:allowsLate?'#166534':'#991b1b',border:`1px solid ${allowsLate?'#bbf7d0':'#fecaca'}`,fontWeight:600,display:'inline-flex',alignItems:'center',gap:'3px'}}>{allowsLate?<Unlock style={{width:10,height:10}}/>:<Lock style={{width:10,height:10}}/>} {allowsLate?'Late allowed':'No late submissions'}</span>}{role==='student'?(isDone&&mySub?<><span className="ex-stat-chip correct" style={{background:'#dcfce7',color:'#15803d',border:'1px solid #86efac',fontWeight:600}}><CheckCircle2 style={{width:12,height:12}}/> Completed · {mySub.score}/{mySub.totalPoints||(count*2)} pts</span><span className="ex-stat-chip correct"><Check style={{width:12,height:12}}/> {mySubStats?.correct} correct</span>{mySubStats&&mySubStats.wrong>0&&<span className="ex-stat-chip wrong"><X style={{width:12,height:12}}/> {mySubStats.wrong} wrong</span>}{mySub.isLate&&<span style={{background:'#fee2e2',color:'#b91c1c',border:'1px solid #fca5a5',fontSize:'0.68rem',padding:'0.15rem 0.5rem',borderRadius:'99px',fontWeight:700}}>LATE</span>}</>:<span className="ex-stat-chip neutral" style={{background:isClosedForStudent?'#fee2e2':due.isPast?'#fef2f2':'#fef3c7',color:isClosedForStudent?'#7f1d1d':due.isPast?'#991b1b':'#92400e',border:`1px solid ${isClosedForStudent?'#fca5a5':due.isPast?'#fecaca':'#fde68a'}`}}>{isClosedForStudent?<Lock style={{width:12,height:12}}/>:<Clock3 style={{width:12,height:12}}/>} {isClosedForStudent?'Submissions closed':due.isPast?'Overdue · Not started':'Not started'}</span>):(hasAnswered?<><span className="ex-stat-chip answered"><Users style={{width:12,height:12}}/> {stats.totalAnswered} answered</span><span className="ex-stat-chip correct"><Check style={{width:12,height:12}}/> {stats.totalCorrect} correct</span><span className="ex-stat-chip wrong"><X style={{width:12,height:12}}/> {stats.totalWrong} wrong</span><span className="ex-stat-chip neutral">{stats.accuracyRate}% accuracy</span></>:<span className="ex-stat-chip neutral"><Users style={{width:12,height:12}}/> 0 answered</span>)}</div><p style={{margin:0,color:'#666',fontSize:'0.88rem'}}>{firstQ}</p></div><div style={{display:'flex',gap:'0.5rem',alignItems:'center'}}>{role==='teacher'&&<Button variant="outline" size="sm" onClick={()=>setActiveAnalyticsEx(ex)} style={{gap:'0.35rem'}}><BarChart3 style={{width:14,height:14}}/> Analytics</Button>}{isClosedForStudent?(<Button variant="outline" size="sm" disabled style={{color:'#7f1d1d',borderColor:'#fca5a5',background:'#fef2f2',cursor:'not-allowed',gap:'4px'}}><Lock style={{width:13,height:13}}/> Closed</Button>):(<Button variant={role==='teacher'?'outline':isDone?'outline':'default'} size="sm" onClick={()=>onStartExercise(ex)} className={role==='student'&&!isDone?'primary-action':''} style={role==='student'&&isDone?{borderColor:'#86efac',color:'#166534',background:'#f0fdf4'}:undefined}>{role==='teacher'?'Preview':isDone?<><Check style={{width:14,height:14,marginRight:4}}/> View result</>:'Start exercise'}</Button>)}</div></div>})}</div>:<div className="class-empty"><FileQuestion/><h3>{role==='teacher'?`Build the first exercise`:'Your teacher is preparing the first activity'}</h3><p>{role==='teacher'?'Start with your own question, then use AI Enhance.':'New lessons and quizzes will appear here.'}</p>{role==='teacher'&&<Button onClick={onQuiz}><Plus/> Create exercise</Button>}</div>}</section><aside className="panel class-stats"><span className="kicker">{role==='teacher'?'Class pulse':'My progress'}</span><h2>{role==='teacher'?(totalClassSubs>0?`${classAccuracy}% accuracy`:`${exercises.length} published`):(exercises.length?`${completedExercisesCount} / ${exercises.length} completed`:'0 completed')}</h2><Progress value={role==='teacher'?(totalClassSubs>0?classAccuracy:(exercises.length?100:0)):studentMasteryPct}/>{role==='teacher'&&totalClassSubs>0&&<div style={{display:'flex',justifyContent:'space-between',marginTop:'1rem',fontSize:'0.78rem',fontWeight:600}}><span style={{color:'#166534'}}><Check style={{width:13,height:13,display:'inline'}}/> {totalClassCorrect} correct</span><span style={{color:'#991b1b'}}><X style={{width:13,height:13,display:'inline'}}/> {totalClassWrong} wrong</span><span><Users style={{width:13,height:13,display:'inline'}}/> {totalClassSubs} learners</span></div>}{role==='student'&&exercises.length>0&&<div style={{display:'flex',justifyContent:'space-between',marginTop:'1rem',fontSize:'0.78rem',fontWeight:600}}><span style={{color:'#166534'}}><CheckCircle2 style={{width:13,height:13,display:'inline'}}/> {completedExercisesCount} completed</span><span style={{color:'#666'}}><Clock3 style={{width:13,height:13,display:'inline'}}/> {exercises.length - completedExercisesCount} remaining</span></div>}<div className="tip" style={{marginTop:'1rem'}}><Sparkles/><p>{role==='teacher'?(totalClassSubs>0?`Class average accuracy is ${classAccuracy}%. Click "Analytics" on any card for question-by-question breakdown.`:'Insights appear after students complete an activity. Check Analytics on each exercise card.'):(completedExercisesCount===exercises.length&&exercises.length>0?'All exercises completed! Great job!':`Keep going! You have completed ${completedExercisesCount} of ${exercises.length} exercise${exercises.length>1?'s':''}.`)}</p></div></aside></div><Dialog open={!!activeAnalyticsEx} onOpenChange={open=>!open&&setActiveAnalyticsEx(null)}><DialogContent className="modal-card analytics-modal"><DialogHeader><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:40,height:40,borderRadius:'50%',background:'#e8f1e9',color:'#173e30',marginBottom:8}}><BarChart3 style={{width:20,height:20}}/></div><DialogTitle>{activeAnalyticsEx?.title} · Analytics</DialogTitle><DialogDescription>Real-time submission results, accuracy rates, and question breakdown.</DialogDescription>{activeAnalyticsEx?.deadline&&(()=>{const d=formatDeadline(activeAnalyticsEx.deadline);const allows=activeAnalyticsEx.allowLateSubmissions!==false;return <div style={{marginTop:6,display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><span style={{display:'inline-flex',alignItems:'center',gap:6,fontSize:'0.76rem',fontWeight:600,color:d.isPast?'#991b1b':'#4b5563',background:d.isPast?'#fef2f2':'#f3f4f6',border:`1px solid ${d.isPast?'#fecaca':'#e5e7eb'}`,padding:'2px 8px',borderRadius:99}}><Calendar style={{width:12,height:12}}/> Due: {d.formatted} {d.isPast?'(Past Due)':''}</span><span style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:'0.75rem',fontWeight:600,color:allows?'#166534':'#991b1b',background:allows?'#f0fdf4':'#fef2f2',border:`1px solid ${allows?'#bbf7d0':'#fecaca'}`,padding:'2px 8px',borderRadius:99}}>{allows?<Unlock style={{width:11,height:11}}/>:<Lock style={{width:11,height:11}}/>} {allows?'Late submissions allowed':'Late submissions blocked'}</span></div>;})()}</DialogHeader>{activeAnalyticsEx&&(()=>{const stats=analyticsMap[activeAnalyticsEx.id]||{totalAnswered:0,totalCorrect:0,totalWrong:0,accuracyRate:0,questionBreakdown:[],submissions:[]};return <div><div className="analytics-summary-grid"><div className="analytics-metric-box"><small>Students Answered</small><strong>{stats.totalAnswered} <span style={{fontSize:'0.85rem',fontWeight:500,color:'#888'}}>/ {currentClass.students||0}</span></strong></div><div className="analytics-metric-box"><small>Class Accuracy</small><strong style={{color:'#15803d'}}>{stats.accuracyRate}%</strong></div><div className="analytics-metric-box"><small>Total Correct</small><strong style={{color:'#166534'}}>{stats.totalCorrect}</strong></div><div className="analytics-metric-box"><small>Total Wrong</small><strong style={{color:'#b91c1c'}}>{stats.totalWrong}</strong></div></div><div style={{marginBottom:'1.5rem'}}><h3 style={{fontSize:'1.05rem',fontWeight:600,marginBottom:'0.75rem'}}>Question-by-Question Performance</h3>{stats.questionBreakdown.length?stats.questionBreakdown.map(q=><div key={q.questionIdx} className="q-breakdown-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}><div><span style={{fontSize:'0.72rem',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.05em',color:'#787268'}}>Question 0{q.questionIdx+1}</span><p style={{margin:'2px 0 6px',fontSize:'0.92rem',fontWeight:500,color:'#111'}}>{q.questionText}</p>{q.expectedAnswer&&<p style={{margin:0,fontSize:'0.78rem',color:'#66786e'}}><strong>Expected Answer:</strong> {q.expectedAnswer}</p>}</div><span style={{fontSize:'0.9rem',fontWeight:700,color:q.accuracyRate>=70?'#15803d':q.accuracyRate>=40?'#d97706':'#b91c1c'}}>{q.accuracyRate}%</span></div><div className="split-bar"><div className="split-bar-fill" style={{width:`${q.accuracyRate}%`}}/></div><div style={{display:'flex',justifyContent:'space-between',marginTop:6,fontSize:'0.72rem',color:'#6b7280'}}><span><Check style={{width:11,height:11,display:'inline',color:'#16a34a'}}/> {q.correctCount} correct</span><span><X style={{width:11,height:11,display:'inline',color:'#dc2626'}}/> {q.wrongCount} wrong</span><span>{q.totalAnswers} answered</span></div></div>):<p style={{color:'#777',fontSize:'0.85rem'}}>No questions found for this exercise.</p>}</div><div><h3 style={{fontSize:'1.05rem',fontWeight:600,marginBottom:'0.75rem'}}>Student Submissions ({stats.submissions.length})</h3>{stats.submissions.length?<div style={{display:'grid',gap:8,maxHeight:'240px',overflowY:'auto'}}>{stats.submissions.map(sub=>{const subStats=computeSubmissionStats(sub,activeAnalyticsEx);const totalPts=sub.totalPoints||(activeAnalyticsEx.questions?.reduce((n:number,q:any)=>n+(Number(q.points)||1),0))||2;return <div key={sub.id} style={{background:'#fff',border:'1px solid #eeeae4',borderRadius:12,padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{display:'flex',alignItems:'center',gap:10}}><span className="avatar" style={{width:32,height:32,fontSize:'0.65rem'}}>{initials(sub.studentName)}</span><div><div style={{display:'flex',alignItems:'center',gap:6}}><b style={{fontSize:'0.85rem'}}>{sub.studentName}</b>{sub.isLate&&<span style={{background:'#fee2e2',color:'#b91c1c',border:'1px solid #fca5a5',fontSize:'0.65rem',padding:'1px 5px',borderRadius:4,fontWeight:700}}>LATE</span>}</div><small style={{color:'#777',display:'block',fontSize:'0.72rem'}}>{sub.studentEmail}</small></div></div><div style={{display:'flex',gap:10,alignItems:'center',fontSize:'0.8rem'}}><span style={{color:'#15803d',fontWeight:600,background:'#f0fdf4',border:'1px solid #bbf7d0',padding:'2px 8px',borderRadius:99,display:'inline-flex',alignItems:'center',gap:4}}><Check style={{width:12,height:12}}/> {subStats.correct} correct</span><span style={{color:'#b91c1c',fontWeight:600,background:'#fef2f2',border:'1px solid #fecaca',padding:'2px 8px',borderRadius:99,display:'inline-flex',alignItems:'center',gap:4}}><X style={{width:12,height:12}}/> {subStats.wrong} wrong</span><span style={{fontWeight:700,background:'#f1ece5',padding:'2px 8px',borderRadius:99}}>{sub.score}/{totalPts} pts</span></div></div>})}</div>:<div className="empty-success" style={{padding:'1.2rem',borderRadius:12,background:'#f8faf7'}}><Users style={{width:20,height:20,margin:'0 auto 6px'}}/> No students have submitted this exercise yet.</div>}</div></div>})()}<DialogFooter style={{marginTop:'1.2rem'}}><Button onClick={()=>setActiveAnalyticsEx(null)}>Close</Button></DialogFooter></DialogContent></Dialog><Dialog open={editOpen} onOpenChange={setEditOpen}><DialogContent className="modal-card"><DialogHeader><DialogTitle>Edit classroom</DialogTitle><DialogDescription>Update details for {currentClass.name}.</DialogDescription></DialogHeader><label className="form-label">Classroom name<Input placeholder="e.g. Mathematics · Form 4" value={editName} onChange={e=>setEditName(e.target.value)}/></label><label className="form-label">Subject<Input placeholder="e.g. Mathematics" value={editSubject} onChange={e=>setEditSubject(e.target.value)}/></label><label className="form-label">Student capacity (Max learners)<Input type="number" min={Math.max(1,currentClass.students||1)} max="100" value={editMaxStudents} onChange={e=>setEditMaxStudents(e.target.value)}/>{currentClass.students>0&&<small style={{color:'#706a63',fontSize:'0.72rem'}}>Currently enrolled: {currentClass.students} student{currentClass.students>1?'s':''}. Capacity cannot be lower.</small>}</label>{editError&&<p className="form-error">{editError}</p>}<DialogFooter><Button variant="outline" onClick={()=>setEditOpen(false)}>Cancel</Button><Button onClick={saveEdit} disabled={savingEdit||!editName.trim()||!editSubject.trim()}>{savingEdit?<LoaderCircle/>:<Check/>} Save changes</Button></DialogFooter></DialogContent></Dialog><Dialog open={deleteOpen} onOpenChange={setDeleteOpen}><DialogContent className="modal-card"><DialogHeader><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:42,height:42,borderRadius:'50%',background:'#fee2e2',color:'#dc2626',marginBottom:10}}><Trash2 style={{width:20,height:20}}/></div><DialogTitle>Delete classroom?</DialogTitle><DialogDescription>Are you sure you want to delete <strong>{currentClass.name}</strong> (Code: <code>{currentClass.code}</code>)?</DialogDescription></DialogHeader><div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:14,padding:'12px 14px',color:'#991b1b',fontSize:'0.8rem',lineHeight:1.5}}><strong>Warning:</strong> This permanently deletes this classroom, its {currentClass.students||0} student membership(s), join requests, and all published exercises. This cannot be undone.</div>{deleteError&&<p className="form-error">{deleteError}</p>}<DialogFooter style={{marginTop:12}}><Button variant="outline" onClick={()=>setDeleteOpen(false)} disabled={deleting}>Cancel</Button><Button onClick={confirmDelete} disabled={deleting} style={{background:'#dc2626',color:'#fff'}}>{deleting?<LoaderCircle/>:<Trash2/>} {deleting?'Deleting…':'Delete classroom'}</Button></DialogFooter></DialogContent></Dialog><Dialog open={!!editExTarget} onOpenChange={open=>!open&&setEditExTarget(null)}><DialogContent className="modal-card"><DialogHeader><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:40,height:40,borderRadius:'50%',background:'#f1ece5',color:'#111',marginBottom:8}}><Pencil style={{width:18,height:18}}/></div><DialogTitle>Edit Exercise Details</DialogTitle><DialogDescription>Update the title, deadline, or late submission policy.</DialogDescription></DialogHeader><label className="form-label">Exercise name<Input placeholder="e.g. Mathematics Checkpoint 1" value={editExTitle} onChange={e=>{setEditExTitle(e.target.value);setEditExError('');}}/></label><label className="form-label" style={{marginTop:'0.8rem'}}>Submission deadline (Optional)<div style={{display:'flex',gap:'8px',alignItems:'center',marginTop:'0.3rem'}}><Input type="datetime-local" value={editExDeadline} onChange={e=>{setEditExDeadline(e.target.value);setEditExError('');}} style={{borderRadius:'10px'}}/>{editExDeadline&&<Button type="button" variant="outline" size="sm" onClick={()=>setEditExDeadline('')} style={{fontSize:'0.75rem',height:'36px'}}>Clear</Button>}</div><small style={{color:'#706a63',fontSize:'0.72rem',display:'block',marginTop:'4px'}}>{editExDeadline?`Due: ${formatDeadline(editExDeadline).formatted}`:'No deadline currently set.'}</small></label>{editExDeadline&&<div style={{marginTop:'0.9rem',padding:'0.8rem 1rem',background:'#fbfaf7',borderRadius:'12px',border:'1px solid #eeeae4'}}><label style={{display:'flex',alignItems:'flex-start',gap:'10px',cursor:'pointer',fontSize:'0.85rem',fontWeight:600,color:'#111'}}><input type="checkbox" checked={editExAllowLate} onChange={e=>setEditExAllowLate(e.target.checked)} style={{marginTop:'3px',accentColor:'#173e30',width:16,height:16}}/><div><span>Allow students to submit after deadline</span><p style={{margin:'2px 0 0',fontSize:'0.75rem',fontWeight:400,color:'#666'}}>{editExAllowLate?'Students can still submit after the deadline, but will be tagged as Late.':'Submissions will be automatically locked after the deadline passes.'}</p></div></label></div>}{editExError&&<p className="form-error">{editExError}</p>}<DialogFooter style={{marginTop:14}}><Button variant="outline" onClick={()=>setEditExTarget(null)}>Cancel</Button><Button onClick={saveEditExercise} disabled={savingEditEx||!editExTitle.trim()}>{savingEditEx?<LoaderCircle/>:<Check/>} Save changes</Button></DialogFooter></DialogContent></Dialog><Dialog open={!!deleteExTarget} onOpenChange={open=>!open&&setDeleteExTarget(null)}><DialogContent className="modal-card"><DialogHeader><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:42,height:42,borderRadius:'50%',background:'#fee2e2',color:'#dc2626',marginBottom:10}}><Trash2 style={{width:20,height:20}}/></div><DialogTitle>Delete exercise?</DialogTitle><DialogDescription>Are you sure you want to delete <strong>{deleteExTarget?.title}</strong>?</DialogDescription></DialogHeader><div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:14,padding:'12px 14px',color:'#991b1b',fontSize:'0.8rem',lineHeight:1.5}}><strong>Warning:</strong> This permanently deletes this exercise and all student submissions for it. This cannot be undone.</div>{deleteExError&&<p className="form-error">{deleteExError}</p>}<DialogFooter style={{marginTop:12}}><Button variant="outline" onClick={()=>setDeleteExTarget(null)} disabled={deletingEx}>Cancel</Button><Button onClick={confirmDeleteExercise} disabled={deletingEx} style={{background:'#dc2626',color:'#fff'}}>{deletingEx?<LoaderCircle/>:<Trash2/>} {deletingEx?'Deleting…':'Delete exercise'}</Button></DialogFooter></DialogContent></Dialog></AppShell>
}
...
function StudentExerciseRunner({user,classroom,exercise,onBack,onExit}:{user:User;classroom:ClassroomData;exercise:any;onBack:()=>void;onExit:()=>void}){
 ...
 useEffect(() => {
  ...
  const checkExisting = async () => {
   try {
    ...
    const directSnap = await getDoc(doc(db, 'classrooms', classroom.id, 'exercises', exercise.id, 'submissions', user.uid));
    if (directSnap.exists() && isMounted) {
     const data = directSnap.data();
     const stats = computeSubmissionStats({ id: directSnap.id, ...data } as SubmissionData, exercise);
    ...
    const qSnap = await getDocs(query(
     collection(db, 'classrooms', classroom.id, 'exercises', exercise.id, 'submissions'),
    ...
    if (!qSnap.empty && isMounted) {
     ...
     const data = d.data();
     const stats = computeSubmissionStats({ id: d.id, ...data } as SubmissionData, exercise);
     setAnswers(data.answers || {});
     setEarnedScore(data.score ?? 0);
     setTotalCorrectState(stats.correct);
     setTotalWrongState(stats.wrong);
 ...
 const isPastDeadline = Boolean(exercise.deadline && (new Date().getTime() > new Date(exercise.deadline).getTime()));
 ...
 const submitExercise = async () => {
  ...
  if (isLateLocked) {
   setSubmitError('The deadline for this exercise has passed, and late submissions are not accepted.');
   return;
  }
  setSubmitting(true);
  setSubmitError('');
  try {
   ...
   const isLate = Boolean(exercise.deadline && (new Date().getTime() > new Date(exercise.deadline).getTime()));
   setIsLateSubmission(isLate);

   // Save using user.uid as document ID so each student has strictly one submission
   await setDoc(doc(db, 'classrooms', classroom.id, 'exercises', exercise.id, 'submissions', user.uid), {
    studentId: user.uid,
   ...
   setSubmitError(e?.message?.includes('permission') ? 'Missing or insufficient permissions. Please refresh your page and try again.' : (e?.message || 'Error submitting exercise.'));
  } finally {
   setSubmitting(false);
  }
 };
  ...
  <AppShell role="student" user={user} onExit={onExit} active="classroom" classCount={1}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
     {dueInfo.formatted && (
      <span style={{
       ...
       alignItems: 'center',
       gap: '5px',
       background: dueInfo.isPast ? (allowsLate ? '#fef2f2' : '#fee2e2') : dueInfo.isUrgent ? '#fef3c7' : '#f3f4f6',
       color: dueInfo.isPast ? (allowsLate ? '#991b1b' : '#7f1d1d') : dueInfo.isUrgent ? '#92400e' : '#374151',
       ...
       {dueInfo.isPast ? (allowsLate ? <AlertCircle style={{ width: 12, height: 12 }}/> : <Lock style={{ width: 12, height: 12 }}/>) : <Calendar style={{ width: 12, height: 12 }}/>}
       {dueInfo.isPast ? (allowsLate ? `Past Due · ${dueInfo.formatted}` : `Closed · ${dueInfo.formatted}`) : `Due ${dueInfo.formatted}`}
      </span>
     )}
     <span className="sdg-pill">{submitted ? 'Completed' : isLateLocked ? 'Closed' : `Question ${currentIdx + 1} of ${rawQuestions.length}`}</span>
    </div>
   </div>
   ...
   {checkingExisting ? (
    <div style={{ textAlign: 'center', padding: '3rem', color: '#777' }}>
     <LoaderCircle className="animate-spin" style={{ width: 32, height: 32, margin: '0 auto 1rem' }}/>
    ...
    <div style={{ background: '#fff', borderRadius: '24px', padding: '2.5rem', boxShadow: '0 0 0 1px #eeeae4', maxWidth: '780px', margin: '0 auto', textAlign: 'center' }}>
     ...
     <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '0.8rem' }}>
      {alreadyCompleted && (
       <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: '#e0f2fe', color: '#0369a1', padding: '0.35rem 0.9rem', borderRadius: 99, fontSize: '0.78rem', fontWeight: 600 }}>
        <CheckCircle2 style={{ width: 14, height: 14 }}/> Previously Completed · One Submission Only
       </div>
      )}
      {isLateSubmission && (
       <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: '#fee2e2', color: '#b91c1c', border: '1px solid #fca5a5', padding: '0.35rem 0.9rem', borderRadius: 99, fontSize: '0.78rem', fontWeight: 700 }}>
        <AlertCircle style={{ width: 14, height: 14 }}/> Submitted After Deadline
       </div>
      )}
     </div>
     <h2 style={{ fontSize: '2rem', fontWeight: 600, margin: '0 0 0.5rem' }}>
      {alreadyCompleted ? 'Exercise Already Completed' : 'Exercise Completed!'}
     </h2>
     <p style={{ color: '#66786e', margin: '0 0 2rem' }}>
      {alreadyCompleted ? 'You have already answered this exercise. Review your results below.' : 'Your answers have been submitted to your teacher.'}
     </p>
     <div style={{ display: 'flex', justifyContent: 'center', gap: '1.2rem', margin: '1.5rem 0 2.5rem', flexWrap: 'wrap' }}>
      <div style={{ background: '#f8faf7', borderRadius: '16px', padding: '1.2rem 1.6rem', minWidth: '130px' }}>
       <small style={{ color: '#778', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.08em', fontWeight: 700 }}>Score</small>
       <div style={{ fontSize: '2rem', fontWeight: 700, color: '#111' }}>{earnedScore} <span style={{ fontSize: '0.9rem', color: '#888' }}>/ {totalPoints}</span></div>
      </div>
      <div style={{ background: '#f8faf7', borderRadius: '16px', padding: '1.2rem 1.6rem', minWidth: '130px' }}>
       <small style={{ color: '#778', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.08em', fontWeight: 700 }}>Mastery</small>
       <div style={{ fontSize: '2rem', fontWeight: 700, color: '#173e30' }}>{Math.round((earnedScore / (totalPoints || 1)) * 100)}%</div>
      </div>
      <div style={{ background: '#f0fdf4', borderRadius: '16px', padding: '1.2rem 1.6rem', minWidth: '130px', border: '1px solid #bbf7d0' }}>
       <small style={{ color: '#15803d', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.08em', fontWeight: 700 }}>Correct</small>
       <div style={{ fontSize: '2rem', fontWeight: 700, color: '#166534' }}>{totalCorrectState}</div>
      </div>
      <div style={{ background: '#fef2f2', borderRadius: '16px', padding: '1.2rem 1.6rem', minWidth: '130px', border: '1px solid #fecaca' }}>
       <small style={{ color: '#b91c1c', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.08em', fontWeight: 700 }}>Wrong</small>
       <div style={{ fontSize: '2rem', fontWeight: 700, color: '#991b1b' }}>{totalWrongState}</div>
      </div>
     </div>

     <div style={{ marginTop: '1.5rem', textAlign: 'left', background: '#fcfbf9', border: '1px solid #eeeae4', borderRadius: '18px', padding: '1.4rem', marginBottom: '2rem' }}>
      <h3 style={{ fontSize: '1rem', fontWeight: 600, margin: '0 0 1rem', color: '#111' }}>Review Your Answers</h3>
      <div style={{ display: 'grid', gap: '1rem' }}>
       {rawQuestions.map((q, i) => {
        const userAns = answers[i] || '';
        const userALow = userAns.trim().toLowerCase();
        const expALow = (q.answer || '').trim().toLowerCase();
        const isCorrect = userALow && expALow && (userALow === expALow || expALow.includes(userALow) || userALow.includes(expALow));
        return (
         <div key={i} style={{ borderBottom: i < rawQuestions.length - 1 ? '1px solid #eeeae4' : 'none', paddingBottom: i < rawQuestions.length - 1 ? '1rem' : '0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
           <span style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#777' }}>Question 0{i + 1}</span>
           <span style={{ fontSize: '0.72rem', fontWeight: 600, color: isCorrect ? '#15803d' : '#b91c1c', background: isCorrect ? '#f0fdf4' : '#fef2f2', border: `1px solid ${isCorrect ? '#bbf7d0' : '#fecaca'}`, padding: '1px 7px', borderRadius: 99 }}>
            {isCorrect ? 'Correct' : 'Needs Review'}
           </span>
          </div>
          <p style={{ margin: '0 0 6px', fontSize: '0.9rem', fontWeight: 500, color: '#111' }}>{q.question}</p>
          <div style={{ fontSize: '0.82rem', color: '#444', background: '#fff', border: '1px solid #e9e4dc', borderRadius: 10, padding: '8px 12px', marginBottom: 4 }}>
           <strong>Your answer:</strong> {userAns || <em style={{ color: '#999' }}>No answer provided</em>}
          </div>
          {q.answer && (
           <div style={{ fontSize: '0.78rem', color: '#166534', marginTop: 3 }}>
    ...
    <div style={{ background: '#fff', borderRadius: '24px', padding: '3rem 2rem', boxShadow: '0 0 0 1px #eeeae4', maxWidth: '640px', margin: '2rem auto', textAlign: 'center' }}>
     <div style={{ width: '64px', height: '64px', background: '#fee2e2', borderRadius: '50%', display: 'grid', placeItems: 'center', margin: '0 auto 1.2rem', color: '#dc2626' }}>
      <Lock style={{ width: '32px', height: '32px' }}/>
     </div>
     <h2 style={{ fontSize: '1.6rem', fontWeight: 600, margin: '0 0 0.5rem', color: '#111' }}>Submissions Closed</h2>
     <p style={{ color: '#666', fontSize: '0.92rem', margin: '0 0 1.5rem', lineHeight: 1.5 }}>
      The deadline for this exercise was <strong>{dueInfo.formatted}</strong>. Your teacher has configured this exercise not to accept late submissions.
     </p>
     <Button onClick={onBack} className="primary-action"><ArrowLeft/> Back to Classroom</Button>
    </div>
   ) : (
...
function QuizBuilder({user,classroom,onBack,onExit}:{user:User;classroom:ClassroomData;onBack:()=>void;onExit:()=>void}){
 ...
 return <AppShell role="teacher" user={user} onExit={onExit} active="quiz" classCount={1}><div className="builder-bar"><button onClick={onBack}><ArrowLeft/> {classroom.name}</button><div><span className="draft-dot"/> {published?'Published':'Draft'} · {title.trim()?<b>{title.trim()}</b>:<em>Untitled Exercise</em>} ({questions.length} Question{questions.length>1?'s':''})</div><Button onClick={publish} disabled={published||publishing||!isValid} className="primary-action">{publishing?<LoaderCircle/>:published?<Check/>:<Send/>} {published?'Published':`Publish (${questions.length})`}</Button></div><div className="builder-head" style={{background:'#fff',border:'1px solid #eeeae4',borderRadius:'22px',padding:'1.8rem',marginBottom:'1.8rem'}}><div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginBottom:'0.5rem'}}><FileQuestion style={{width:18,height:18,color:'#173e30'}}/><span className="kicker" style={{margin:0}}>Exercise Setup</span></div><label className="form-label" style={{fontSize:'0.95rem',fontWeight:800,marginTop:'0.4rem'}}>Exercise Name / Title <span style={{color:'#dc2626'}}>*</span><Input placeholder="e.g. Mathematics Chapter 3 Practice, Form 4 Quadratic Equations" value={title} onChange={e=>{setTitle(e.target.value);setTitleError('');}} style={{fontSize:'1.15rem',fontWeight:600,height:'48px',borderRadius:'14px',marginTop:'0.45rem',background:'#fcfbf9',border:titleError?'2px solid #ef4444':'1px solid #dce5dc'}}/></label>{titleError?<p style={{color:'#dc2626',fontSize:'0.78rem',fontWeight:600,margin:'0.45rem 0 0'}}>{titleError}</p>:<p style={{fontSize:'0.82rem',color:'#6e7e75',margin:'0.45rem 0 0'}}>Enter a clear title for this exercise so your learners recognize it in the classroom.</p>}<div style={{marginTop:'1.2rem',paddingTop:'1.2rem',borderTop:'1px solid #f0ede6'}}><label className="form-label" style={{fontSize:'0.9rem',fontWeight:700,display:'block',marginBottom:'0.4rem'}}><div style={{display:'flex',alignItems:'center',gap:'6px'}}><Calendar style={{width:15,height:15,color:'#173e30'}}/> Submission Deadline (Optional)</div><div style={{display:'flex',alignItems:'center',gap:'10px',marginTop:'0.45rem',flexWrap:'wrap'}}><Input type="datetime-local" value={deadline} onChange={e=>setDeadline(e.target.value)} style={{maxWidth:'320px',borderRadius:'12px',background:'#fcfbf9'}}/>{deadline&&<Button type="button" variant="outline" size="sm" onClick={()=>setDeadline('')} style={{fontSize:'0.78rem',height:'38px',borderRadius:'10px'}}>Clear deadline</Button>}</div></label><p style={{fontSize:'0.8rem',color:'#6e7e75',margin:'0.4rem 0 0'}}>{deadline?`Learners will see this due date: ${formatDeadline(deadline).formatted}.`:'No deadline set. Learners can complete this activity at any time.'}</p>{deadline&&<div style={{marginTop:'0.9rem',padding:'0.8rem 1rem',background:'#fbfaf7',borderRadius:'12px',border:'1px solid #eeeae4'}}><label style={{display:'flex',alignItems:'flex-start',gap:'10px',cursor:'pointer',fontSize:'0.85rem',fontWeight:600,color:'#111'}}><input type="checkbox" checked={allowLateSubmissions} onChange={e=>setAllowLateSubmissions(e.target.checked)} style={{marginTop:'3px',accentColor:'#173e30',width:16,height:16}}/><div><span>Allow students to submit after deadline</span><p style={{margin:'2px 0 0',fontSize:'0.75rem',fontWeight:400,color:'#666'}}>{allowLateSubmissions?'Students can still submit after the deadline, but will be tagged as Late.':'Submissions will be automatically locked after the deadline passes.'}</p></div></label></div>}</div></div><div className="builder-grid"><section className="question-editor" style={{display:'grid',gap:'1.5rem'}}>{questions.map((q,idx)=><div key={q.id} style={{borderBottom:idx<questions.length-1?'1px solid #e9eee9':'none',paddingBottom:idx<questions.length-1?'1.5rem':'0'}}><div className="question-count"><span>Question 0{idx+1}</span>{questions.length>1&&<button type="button" onClick={()=>removeQuestion(idx)} aria-label="Remove question" style={{border:0,background:'none',color:'#a00',cursor:'pointer'}}><X style={{width:'18px',height:'18px'}}/></button>}</div><label className="form-label">Question text<Textarea value={q.question} onChange={e=>updateQuestion(idx,'question',e.target.value)} placeholder="Type question prompt..." className="question-text"/></label><div className="answer-block"><label className="form-label">Answer<Input value={q.answer} onChange={e=>updateQuestion(idx,'answer',e.target.value)} placeholder="Correct answer or explanation"/></label><label className="form-label">Points<Input type="number" min="1" value={q.points} onChange={e=>updateQuestion(idx,'points',Number(e.target.value)||1)}/></label></div><div className="hint-row"><div><span><Bot/></span><p><b>AI enhancement</b><small>Add context and scaffold reasoning.</small></p></div><Button className="enhance-btn" onClick={()=>enhanceQuestion(idx)} disabled={q.loading}>{q.loading?<><LoaderCircle/> Enhancing…</>:<><WandSparkles/> AI Enhance</>}</Button></div>{q.enhanced&&<div className="enhanced-note"><CheckCircle2/><div><b>Question 0{idx+1} enhanced</b><p>Added contextual problem framing & step-by-step reasoning prompt.</p></div></div>}</div>)}<button type="button" onClick={addQuestion} className="add-question" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'0.5rem',padding:'1rem',width:'100%',cursor:'pointer'}}><Plus style={{width:'18px',height:'18px'}}/> Add Question 0{questions.length+1}</button><div style={{marginTop:'1rem',display:'flex',justifyContent:'flex-end',gap:'0.8rem'}}><Button variant="outline" onClick={onBack}>Cancel</Button><Button onClick={publish} disabled={published||publishing||!isValid} className="primary-action">{publishing?<LoaderCircle/>:published?<Check/>:<Send/>} {published?'Published to Classroom':publishing?'Publishing...':`Publish Exercise (${questions.length} Question${questions.length>1?'s':''})`}</Button></div></section><aside className="ai-sidebar"><div className="ai-sidebar-head"><span><Sparkles/></span><div><small>SLearn AI</small><h3>Exercise overview</h3></div></div><div className="quality-score"><div><strong>{questions.length}</strong><span>Questions</span></div><p>Exercise Size</p></div><div className="checks">{['Clear learning objective','Age-appropriate language','Real-world relevance','Guided reasoning'].map((x,i)=><div className={(hasEnhancedAny||i<2)?'ready':''} key={x}><span>{hasEnhancedAny||i<2?<Check/>:i+1}</span>{x}</div>)}</div></aside></div></AppShell>
}

export default function Home(){
 const [user,setUser]=useState<User|null>(null),[role,setRole]=useState<Role|null>(null),[ready,setReady]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(''),[view,setView]=useState<View>('dashboard'),[selectedClass,setSelectedClass]=useState<ClassroomData|null>(null),[selectedExercise,setSelectedExercise]=useState<any|null>(null),[,setProfileVersion]=useState(0);
 ...
 useEffect(()=>{const navigate=(event:Event)=>{const target=(event as CustomEvent<NavTarget>).detail;setView('dashboard');setSelectedClass(null);setSelectedExercise(null);setTimeout(()=>{if(target==='overview')window.scrollTo({top:0,behavior:'smooth'});else document.getElementById(target)?.scrollIntoView({behavior:'smooth',block:'start'})},40)};const refreshProfile=()=>setProfileVersion(v=>v+1);window.addEventListener('slearn:navigate',navigate);window.addEventListener('slearn:profile-updated',refreshProfile);return()=>{window.removeEventListener('slearn:navigate',navigate);window.removeEventListener('slearn:profile-updated',refreshProfile)}},[]);
