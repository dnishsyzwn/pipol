...
import { AlertCircle, ArrowLeft, ArrowRight, BarChart3, BookOpen, Bot, Calendar, Check, CheckCircle2, ChevronRight, Clock3, Copy, FileQuestion, GraduationCap, LayoutDashboard, LoaderCircle, Lock, LogOut, Menu, MoreHorizontal, Pencil, Plus, Search, Send, Sparkles, Trash2, Unlock, Users, WandSparkles, X } from 'lucide-react';
...
export default function Home(){
 ...
 const login=async(selectedRole:Role)=>{setBusy(true);setError('');try{const result=await signInWithPopup(auth,googleProvider);const userRef=doc(db,'users',result.user.uid);const existingSnap=await getDoc(userRef);if(existingSnap.exists()){const data=existingSnap.data();const existingRole=data?.role as Role|undefined;if(existingRole&&existingRole!==selectedRole){await signOut(auth);setError(`This Google account (${result.user.email}) is already registered as a ${existingRole.toUpperCase()}. You cannot sign in or create an account as a ${selectedRole.toUpperCase()} with this email.`);return;}await setDoc(userRef,{displayName:result.user.displayName||data?.displayName||'',email:result.user.email||data?.email||'',photoURL:result.user.photoURL||data?.photoURL||'',lastLoginAt:serverTimestamp()},{merge:true});setUser(result.user);setRole(existingRole||selectedRole);setView('dashboard')}else{const emailSnap=await getDocs(query(collection(db,'users'),where('email','==',result.user.email),limit(1)));if(!emailSnap.empty){const otherRole=emailSnap.docs[0].data()?.role;if(otherRole&&otherRole!==selectedRole){await signOut(auth);setError(`This email (${result.user.email}) is already registered as a ${otherRole.toUpperCase()}. You cannot create a ${selectedRole.toUpperCase()} account.`);return;}}await setDoc(userRef,{displayName:result.user.displayName||'',email:result.user.email||'',photoURL:result.user.photoURL||'',role:selectedRole,createdAt:serverTimestamp(),lastLoginAt:serverTimestamp()});setUser(result.user);setRole(selectedRole);setView('dashboard')}}catch(e){setError(friendlyError(e))}finally{setBusy(false)}};
